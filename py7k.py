...
#description:   python module to read a RESON 7k file format
...
class s7kreader:
	...
	packetheader_fmt = '=2H4L2Hf2bH2LHHLHHLLL' #do not include anything beyond the total records in fragmented data record set.  stop header at the 2 reserved words
	# packetheader_fmt = '=2H4L2HF2HH2L2HL' #do not include anything beyond the total records in fragmented data record set.  stop header at the 2 reserved words
	...
	def readdatagram(self):
		...
		if recordtypeidentifier == 1003: 
			dg = P_1003(self.fileptr, numberOfBytes, recorddate)
...
class P_1003:
	def __init__(self, fileptr, numberOfBytes, recorddate):
		self.recordtypeidentifier = '1003'	# assign the code for this datagram type
		self.offset = fileptr.tell()	# remember where this packet resides in the file so we can return if needed
	...
	def read(self):
		self.fileptr.seek(self.offset, 0)# move the file pointer to the start of the record so we can read from disc
		hdr = s7kreader().packetheader_fmt
		rec_fmt = hdr + 'Lfddd4B'
		rec_len = struct.calcsize(rec_fmt)
		rec_unpack = struct.Struct(rec_fmt).unpack
		# bytesRead = rec_len
		...
		self.datumidentifier 	= s[22]
		self.latency	  		= s[23]
		self.latitude			= float (s[24]) * 180 / math.pi
		self.longitude			= float (s[25]) * 180 / math.pi
		self.height		 		= float (s[26])
		self.positiontypeflag 	= s[27]
		self.utmzone			= s[28]
		self.qualityflag		= s[29]
		self.positionmethod		= s[30]
